...
def callback(wii_joy):
    global angXX, angYY, angZZ, shiftXX, shiftYY, shiftZZ, shiftXX0, flag_w_s, flag_m, div_angZZ, cnt_w_s, cnt_m

    print(wii_joy.data[0], wii_joy.data[1], wii_joy.data[2], wii_joy.data[3])
    ...
    if flag_w_s == True:
        if flag_m == False:
            shiftXX = -float(wii_joy.data[3])
            angYY = -float(wii_joy.data[2])/5
            print("Walk Turn Mode", flag_w_s, flag_m, shiftXX, angYY)
        if flag_m == True:
            shiftXX = -float(wii_joy.data[3])
...
def talker(shiftX, shiftY, shiftZ, angX, angY, angZ):
    ...
    rospy.init_node('s6_12axis_angle_control', anonymous=True)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            talker(shiftXX, shiftYY, shiftZZ, div_angXX, angYY, div_angZZ)
